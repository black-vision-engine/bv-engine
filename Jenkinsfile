...
def generateDoxygenDocs( buildDir, conf, platform )
{
    ...
    {
        bat "$\"${tool 'TortoiseHg'}\\hq.exe\" clone https://nieznanysprawiciel@bitbucket.org/blackvision/bv_engine/wiki --config auth.jenkins.prefix=* --config ${env.USERNAME} --config ${env.PASSWORD}"
    }
