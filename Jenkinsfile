...
def generatePerformancePlots( buildDir, conf, platform )
{
    ...
    for (i = 0; i < plotFiles.size(); i++)
    {
        ...
        plot csvFileName: "${file.name}", csvSeries: [[displayTableFlag: true, exclusionValues: 'meanTime,medianTime,meanCPU,medianCPU', file: "${file}", inclusionFlag: 'INCLUDE_BY_STRING', url: '']], exclZero: false, group: 'Performance', keepRecords: true, logarithmic: false, numBuilds: '', style: 'line', title: "${file.name}", useDescr: true, yaxis: '', yaxisMaximum: '', yaxisMinimum: ''
    }
}
