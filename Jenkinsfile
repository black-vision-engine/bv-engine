...
def generateDoxygenDocs( buildDir, conf, platform )
{
    ...
    {
        bat "\"${tool 'TortoiseHg'}\\hg.exe\" https://nieznanysprawiciel@bitbucket.org/blackvision/bv_engine/wiki --debug --config auth.jenkins.prefix=* --config ${env.USERNAME} --config ${env.PASSWORD} --config \"auth.jenkins.schemes=http https\" clone"
    }
