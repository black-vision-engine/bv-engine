...
def generateDoxygenDocs( buildDir, conf, platform )
{
    ...
    {
        bat "\"${tool 'TortoiseHg'}\\hg.exe\" https://nieznanysprawiciel@bitbucket.org/blackvision/bv_engine/wiki --debug --config auth.jenkins.prefix=* --config auth.x.username=${env.USERNAME} --config auth.x.password=${env.PASSWORD} --config \"auth.jenkins.schemes=http https\" clone"
    }
