...
// *********************************
//
FontAtlasCacheEntryPtr      FontAtlasCache::GetEntry        ( const std::string & fontName, SizeType fontSize, SizeType blurSize, SizeType glowBlurSize, 
                                                              SizeType outlineWidth, bool withMipMaps, const std::wstring & charSet )
{
    std::string sql = "SELECT * FROM " + m_sCachedFontsTableName + " WHERE " +
        ...
        " AND " + FontAtlasCacheEntry::CN::GLOW_BLUR_SIZE + " = " + std::to_string( glowBlurSize ) +
