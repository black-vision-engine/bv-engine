...
void                                DefaultTexturePlugin::Update                      ( TimeType t )
{
	...
	auto wY = GetWrapModeY();
	auto fm = GetFilteringMode();
    auto bs = GetBorderColor();

	if ( m_prevPlugin->GetVertexAttributesChannel()->NeedsAttributesUpdate() || StateChanged( wX, wY, fm, attachmentMode, bs ) )
	{
		UpdateState( wX, wY, fm, attachmentMode, bs );
...
// *************************************
// 
bool                                        DefaultTexturePlugin::StateChanged                ( TextureWrappingMode wmX, TextureWrappingMode wmY, TextureFilteringMode fm, TextureAttachmentMode am, glm::vec4 bc ) const
{
	return wmX != m_lastTextureWrapModeX || wmY != m_lastTextureWrapModeY || fm != m_lastTextureFilteringMode || am != m_lastTextureAttachMode || bc != m_lastBorderColor;
}

// *************************************
// 
void                                        DefaultTexturePlugin::UpdateState                 ( TextureWrappingMode wmX, TextureWrappingMode wmY, TextureFilteringMode fm, TextureAttachmentMode am, glm::vec4 bc )
{
    ...
    m_lastBorderColor           = bc;

    auto textures = m_psc->GetTexturesDataImpl()->GetTextures();
    for( auto texture : textures ) // TODO: is that really what we want?
    {
        auto txDesc = reinterpret_cast< DefaultTextureDescriptor* >(texture);
	    txDesc->SetBorderColor( bc );
    }
}
