...
// *********************************
//
void	RenderLogic::SetVideoCardManager( videocards::VideoCardManager * videoCardManager )
{
	...
	if(m_VideoCardManager->GetVideoCardsSize()==0)
	{
		...
		for(unsigned int i = 0   ;   i < m_VideoCardManager->GetVideoCardsSize() ; i++)
		{
			if( m_VideoCardManager->GetVideoCard(i)->GetBrand() == "BlueFish" )
			{
				...
				m_VideoCardManager->m_VideoCardConfig.m_BlueFishConfig.push_back( videocards::VideoCardConfig() );
...
void    RenderLogic::PushToVideoCard  ( Texture2DConstPtr frame ) // FIXME: pablito source code.
{
	...
	{
		if( m_VideoCardManager->m_CurrentTransferMode == videocards::VideoCard_RAM_GPU::GPU )
		{          
			//m_offscreenRenderLogic->TransferFromGPUToSDI( renderer, m_VideoCardManager );
			//m_offscreenRenderLogic->SwapDisplayRenderTargets();
			//todo: fix gpu direct
		}
		else if( m_VideoCardManager->m_CurrentTransferMode == videocards::VideoCard_RAM_GPU::RAM )
		{
			m_VideoCardManager->GetBufferFromRenderer( frame->GetData( 0 ) );
		}
	}
